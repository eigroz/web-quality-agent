  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100">
      ...
      <main className="mx-auto max-w-7xl px-6 py-12 lg:px-10 lg:py-16">
        ...
        {!journey && <ExecutivePreview market={countries.find((option) => option.code === country)?.name ?? country} targetUrl={url} />}
        ...
        {journey && <section className="mt-5 border border-slate-800 bg-slate-900 p-5"><p className="text-xs font-bold tracking-[0.2em] text-sky-400 uppercase">Repeatable checks</p><p className="mt-2 text-sm text-slate-400">These findings focus on patterns that should stay the same between runs, not small changes in network timing. Click a finding to see the resources causing it.</p><div className="mt-4 space-y-2">{journey.steps.flatMap((step) => step.loaded.duplicateResources.map((resource) => ({ step: step.name, resource }))).map((finding) => <details key={`${finding.step}-${finding.resource.url}`} className="border border-slate-800 bg-slate-950 p-3"><summary className="cursor-pointer text-sm text-slate-300"><span className="font-semibold text-sky-300">Duplicate load:</span> {finding.resource.role} loaded {finding.resource.count} times on {finding.step}</summary><div className="mt-3 grid gap-4 border-t border-slate-800 pt-3 text-xs text-slate-400 md:grid-cols-[1fr_280px]"><div><p className="break-all text-slate-300">{finding.resource.url}</p><p className="mt-2">Total transferred: {(finding.resource.transferSize / 1024).toFixed(1)} KB | Total request time: {(finding.resource.durationMs / 1000).toFixed(2)}s</p><p className="mt-2">Technical label: {finding.resource.vendor ? `${finding.resource.vendor} / ` : ""}{finding.resource.purpose}</p></div><div className="border-l-2 border-sky-500/60 pl-3"><p className="font-semibold text-sky-300">What this is</p><p className="mt-1 text-slate-300">{finding.resource.role}</p><p className="mt-3 text-amber-200">Why it matters: the browser fetched the same resource repeatedly, using extra data and work. Check whether it can be cached, loaded once, or removed.</p></div></div></details>)}{journey.steps.every((step) => step.loaded.duplicateResources.length === 0) && <p className="text-sm text-emerald-300">No duplicate resource loads were detected.</p>}</div></section>}
